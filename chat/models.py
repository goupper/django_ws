...
from django.contrib.auth.models import (
    AbstractUser, AbstractBaseUser, Permission, PermissionsMixin, UnicodeUsernameValidator, BaseUserManager)
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    vip_recommend_id = models.SmallIntegerField(
        default=0, blank=True, verbose_name='vip等级',
        help_text='-1: 会员过期,0: 非会员,大于0: 具体的会员等级'
    )
    user_type = models.PositiveSmallIntegerField(
        default=1, blank=True, choices=((1, 'Normal'), (2, 'Sale')),
        verbose_name='用户类型'
    )
    gender = models.PositiveSmallIntegerField(
        default=1, blank=True, choices=((1, '男'), (2, '女')),
        verbose_name='性别'
    )
    login_time = models.DateTimeField(
        blank=True, null=True, verbose_name='登录时间'
    )
...
class RoomUser(AbstractModel):
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='user_rooms'
    )
    room = models.ForeignKey(
        Room, on_delete=models.CASCADE, related_name='room_users'
    )
    is_owner = models.BooleanField(
        default=False, blank=True, verbose_name='Is Owner'
    )
